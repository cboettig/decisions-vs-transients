...
arima_forecast %>% filter(t < 2000) %>%
  ggplot(aes(t)) + 
  geom_line(aes(y=x), col="white") +
  geom_line(aes(y=pred), col="grey", lwd = 1) + 
  geom_ribbon(aes(y=pred, ymin=pred-se, ymax=pred+se), fill="white", alpha = 0.5) + 
...
---
layout: true
background-image: linear-gradient(to right, rgba(50, 50, 50, .8), rgba(50, 50, 50, .9)), url(img/royalsociety/snake.jpg)
background-size: 120%
background-position: center
...
---
background-image: linear-gradient(to right, rgba(50, 50, 50, .8), rgba(50, 50, 50, .9)), url(img/royalsociety/reef-shark.jpg)
background-size: 120%
class: top, inverse
background-position: center
layout: true
...
discrete_sims %>% filter(series %in% c("1", "2")) %>%
  ggplot(aes(time,state)) + geom_line(col="white", lwd=1.5) + facet_wrap(~ series)
```
...
vp <- grid::viewport(width = .5, height = 0.55, x = 0.35, y = 0.7)
...
---
layout: true
background-image: linear-gradient(to right, rgba(30, 30, 30, .8), rgba(30, 30, 30, .9)), url(img/royalsociety/forest-fire.jpg)
background-size: 120%
class: center, top, inverse
...
df %>% mutate(state = states[state], action = actions[action]) %>% 
  ...
  geom_point(col="white")+
  geom_line(aes(time, action), col="grey", lwd=2) + ylim(0,1.4)
```
...
data.frame(a = possible_a, probability = prior) %>%
  ggplot(aes(a,prior)) + geom_bar(stat="identity", fill="white") +
  geom_vline(aes(xintercept = true_a), col="white", lwd=1, lty=2) 
```
...
![](img/royalsociety.jpg) Photo Competition 
]
.pull-right[
