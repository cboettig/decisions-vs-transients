...
## Time-series of the dynamics from the deterministic skeleton
...
#  First-passage times to the stable attractor
...
passage %>% ggplot(aes(tau)) + geom_histogram(binwidth = 200, fill=rgb(.95,.95,.95)) + geom_vline(aes(xintercept=det_tau), col="blue", lwd=2) #+ theme_bw(42)
...
# Features that drive long transient dynamics also create stochastic phenomena

- Emergent ensemble dynamics not predicted by the deterministic model
- ... But intuition is simple: noise helps 'jump' the ghost
- .... But also creates dynamics that are deeply challenging for both inference & decision-making

---
background-image: linear-gradient(to right, rgba(50, 50, 50, .8), rgba(50, 50, 50, .95)), url(img/royalsociety/reef-shark.jpg)
background-size: 120%
class: center, top, inverse
background-position: center
...
Given time-series observations, can 
