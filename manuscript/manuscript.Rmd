...
Where $X_t$ is the population density of the focal species (the beetle, in our case).  The growth  rate of the species is determined by an intrinsic growth rate $r$ and carrying capacity $K$. The species is under mortality pressure from another species (the natural defenses of trees, in our context) which has an S curve saturation which reaches its half maximum at $H$ and maximum rate of $a$. Parameter $Q$ determines the steepness of the saturating response. The dynamics are subject to environmental white noise proportional to the population size, $\xi_t X_t$, uncorrelated Gaussian noise $\xi \sim \mathcal{N}(0,\sigma)$ with mean zero and standard deviation $\sigma$ [@Boettiger2018]. Model dynamics are illustrated in Fig 1 under two different parameter choices for the saturation response $a$: one which corresponds to a ghost attractor and one to a bistable system.
