...
```{r fig_policy, fig.cap="Optimal policy functions under each model. Parameter values for utility function used here are $c = 1$, $d = 0.5$, $X_0 = 50$, $\delta = 0.999$, parameters for the transition models are as given in Fig 1. "}
## Fig 3
...
Long transients. Equilibrium.



Note that this approach can equally well consider an arbitrary suite of models (i.e. varying the parameter $a$ over a range of values, as well as to models with completely different structure).

Not considered:

- partially observed optimal control
- non-stationary dynamics e.g. in which 

Implications: 

- Long transients can be misleading -- e.g. mistaking a ghost attractor for a stable state.
- These mistakes can be significant, since long transients require qualitatively different management strategies.
- Nevertheless, thes
