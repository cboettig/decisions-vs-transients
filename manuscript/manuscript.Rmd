...
Systems with alternative stable states which can exhibit critical transitions are of great theoretical interest [@Scheffer2001; @Ratajczak2018] and pose unique challenges to ecological management [@Folke2004; @Fischer2009; @Polasky2011].  Most of this work has focused on critical transitions driven by saddle-node bifurcations [@Boettiger2013], which produce a blue-sky transition between alternative stable states and exhibit hysteresis which can frustrate efforts to restore the pre-transition state. Less attention has been given to the dynamics which immediately follow the saddle node bifurcation, in which the effects of the anhilated stable state may still be felt as a "Ghost Attractor," as introduced in the molecular and neuroscience literature [e.g. @Grossberg1980; @Deco2012], and more recently in the ecological literature [@Hastings2018].  As such, despite the relative simplicity of the model, Ghost Attractor dynamics can provide important insights to our understanding of long transients, alternative stable state dynamics,and critical transitions, and the consequences such phenomena have for ecological management.  
...
\begin{equation}
X_{t+1} = g(X_t) = X_t + \underbrace{X_t r \left(1 -\frac{X_t}{K} \right)}_{\textrm{logistic growth}}  - 
...
We can extend this model to include the application of a control action in year $t$ of $A_t$ that decreases the population by a fixed amount before reproduction occurs, such that:

\begin{equation}
X_{t+1} = f(X_t, A_t) = g(X_t - A_t)
\end{equation}

Thus with $A_t = 0$ we can model the dynamics without control, and can model any control policy over time as a set of actions $A_t$ describing the beetle mortality imposed by management response that year.
...
To properly understand the consequences transient dynamics have for management, we must first be more explicit about both the management objectives and potential management actions.  For simplicity, I will imagine a manager has the option to apply a direct mortality to the beetle larvae prior to recruitment, e.g. by spraying at some given intensity. Thus the action space is defined by some measure of spraying effort between 0% and 100%.  The management objectives are defined by a utility function, which trades off the costs of control with the costs of damage due to the outbreak.  Here I assume linearly increasing cost of control (fixed price per unit spraying). Below an endemic population size $X_0$ I assume damage (e.g. no tree mortality) is minimal, and then increases quadratically with increasing beetle density; that is, the instantaneous utility of state with beetle density $X_t$ and taking action $A_t$ (the "utility function") is

\begin{equation}
U(X_t, A_t) = \underbrace{ - d \max\left(X_t - X_0, 0\right)^2}_{\textrm{damage}} - \underbrace{c A_t}_{control} \label{utility}
\end{equation}

Where `d` is the coefficient representing the damage (in terms of lost utility), and `c` is the coefficient representing the cost to providing control.  Note that while it is common to express utility in strictly monetary terms (i.e. damage to the forest expressed in dollars lost, vs dollars spent to control the outbreak), it is by no means an assumption of the approach that utility must maximize monetary reward.  In particular, different managers may assign different utility to either forest state or the control effort (i.e. differ in their values of `d` and `c`), and thus reach different management strategies, both of which are 'optimal' relative to their separate utilities.  See @Walters1986, @Clark1990, @Fischer2009, and @Marescot2013 for further discussion of utility maximization in ecological management.
...
```{r figure3, fig.cap="Optimal policy functions under each model. A policy function indicates the optimal action (y-axis) that should be taken given each possible state (x-axis) of the system. The policy function arises as the solution to the Hamilton-Jacobi-Bellman recursion, Eq (4), e.g. as found by stochastic dynamic programming. Parameter values for utility function used here are $c = 1$, $d = 0.5$, $X_0 = 50$, $\\delta = 0.999$, parameters for the transition models are as given in Fig 1."}
## Fig 3
...
fixed_sims <- 
 bind_rows("simulations under ghost policy function" = sim_ghost, 
           "simulations under bistable policy function" = sim_bistable, 
           .id = "prior") %>% 
...
Note that in the management scenario stipulated, we have not assigned an intrinsic value to preserving beetles (or equivalently, a penalty for eliminating the beetle population entirely).  Consequently, once the manager concludes the endemic state is no longer stable but is merely a transient created by the ghost attractor, elimination of the beetles is the best way to maximize the particular utility function chosen for this example.  Given a different utility function, the outcome of the adaptive management process could be quite different.  While our utlity function has been chosen for illustrative purposes, in practice a utility function would be determined through a delibrate process of stakeholder engagement.
...
# Fig 7
learning <- list(df = read_csv("manuscript_cache/learning_df.csv"), 
                 posterior = read_csv("manuscript_cache/learning_posterior.csv"))
 learning$df %>% 
  select(-value, -obs) %>% 
...
```{r figure8, fig.cap="Learning through Adaptive Management of the forest-beetle system.  Initial probabilities for each model are equal, and learning proceeds over the next six timesteps (shown in lighter shades in decreasing probability of the bistable model beign correct).  By the sixth iteration, the manager has opted to eliminate the beetles and further learning is no longer possible in this system."}
model_names <- c("Ghost", "Bistable")
learning$posterior %>% 
  data.frame(time = 1:Tmax) %>%
  filter(time < 12) %>%
  gather(model, probability, -time, factor_key =TRUE) %>% 
  mutate(model = model_names[as.integer(model)]) %>% 
  ggplot(aes(x = time, y = probability, col = model)) + geom_point() + 
  geom_line()
#  geom_bar(aes(x = param, y = probability, group = time, alpha = time), stat="identity", position = "identity", show.legend = FALSE, fill=pal[1]) +  xlab("Model")
```
