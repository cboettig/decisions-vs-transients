...
In the remaining sections, I explore two different approaches to addressing the problem of management under a potential ghost attractor.  The first considers the decision-theoretic optimal policy given uncertainty 
...
```{r eval=FALSE}
## Figure not shown -- intermediate prior gives an intermediate policy
tibble(state = states,
...
## Rather stochastic in realization space, should consider ensembles instead...
reps <- 10
Tmax <- 60
set.seed(123456)
...
sims_by_prior %>% 
  ...
  geom_path(alpha=0.5) + facet_wrap(~prior)
```
