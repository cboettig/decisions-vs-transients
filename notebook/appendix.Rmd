...
p <- list(r = .8, K = 153, q = 2, b = 20, sigma = .02, a=2.3,
          N = 3e3, x0 = 20)
#eps <- states[2]/10

may <- function(a){  
  function(x, h = 0){ # May
    x + x * p$r * (1 - x / p$K)  - a * x ^ p$q / (x ^ p$q + p$b ^ p$q) - h
  }
}

reward_fn <- function(x,h) - damage * (x / max(states)) ^ 2 - control * h / max(actions)
...
possible_a <- seq(2*true_a - believe_a, 2*believe_a - true_a, by = 0.1)
...
```{r}
samples <- examples %>% group_by(series) %>% group_modify(~estimate_posterior(.x))
```
...
```{r}
soln <- mdp_value_iteration(m_belief, reward, discount)
opt_soln <- mdp_value_iteration(m_true, reward, discount)
...
```{r}
tibble(state = states,
       belief = actions[soln$policy],
       ideal =  actions[opt_soln$policy]) %>%
...
set.seed(12345)
df <- mdp_planning(m_true, reward, discount, model_prior = c(1), 
                   policy = soln$policy, x0 = x0, Tmax = 200)
...
df <- mdp_planning(m_belief, m$reward, discount, model_prior = c(1), 
...
df <- mdp_planning(m_true, reward, discount, model_prior = c(1), 
                   policy = opt_soln$policy, x0 = x0, Tmax = 200)
